...
1.  If you are not already a member of the [ROSSyndicate GitHub
...
Elect to start the repository with a README.md file and a MIT use license file.
After you've initialized your repository, go in and edit the third line of the
license file to read: *'ROSS - Radical Open Science Syndicate and all
contributors'* 2.  Add collaborators with write/push access by going to repo
Settings -\> Manage Access -\> Invite a Collaborator. 3.  After you have forked
the repository to your account, clone the repo to your local machine (see
Forking and Cloning ROSSyndicate Repositories). 4.  Once the repo is stored on
your local machine create the general architecture for your repository.
    ...
    types that you may wish to include at the initiation of your project.
        ...
        internal project, this should be a SymLink folder to a OneDrive folder 
        (see 'Data and OneDrive') . For external projects/deliverables (with 
        datasets that do not trigger large-file errors \>100mb), this can be a 
        folder that contains data files that travel with and are tracked by the 
        repository.
        ...
        This could include figures, summary data, etc. Your out/output folder 
        could also live within the data folder, depending on the size of your project output.
    -   Populate the README.md with some basic information, including the primary 
    contact for the repo, and what the repo is all about. Also include the 
    following use license blurb: 
    *'This repository is covered by the MIT use license. We request that all downstream uses of this work be available to the public when possible.'*
    ...
    pull request to your creative partner (see 'Collaboration workflow overview').
...
account, which is where you will make changes and edits prior to making a pull
request to incorporate your changes to the primary repository.
...
command-line interpreter.
    ...
    cloned repository to be stored. See this 
    [tutorial on command-line basics](https://www.davidbaumgold.com/tutorials/command-line/) 
    so you know how to navigate at the terminal. Most folks in the lab create a 
    'GitHub' directory within our Documents folder, but we don't have any required 
    location for where you store your local repository.

    -   Then, clone your repo! You can acquire the `ssh_address` from your forked 
    repository on the GitHub website. If I were cloning the repo below, I would 
    replace `ssh_address` with `git@github.com:steeleb/ROSSyndicate_best_practices`
...
repository by typing the following in the terminal. This is part of the way we
avoid merge conflicts down the road.

    > git fetch upstream

2.  Work on your local repo, commit, edit, commit, edit, commit. See 'Commit
history best practices' for advice on great commits. Commits are made in your
...
3.  Now, stage the files for your commit.
    ...
    If you are happy with the files listed under 'Changes to be committed', 
    make a commit!
...
6.  Craft and submit your PR!
    ...
    conforms to the 'Pull Request best practices' recommendations. Pull request 
    descriptions and comments are written in Markdown, so take advantage of 
    embedding images, emojis, text blocks, etc. You can also associate specific 
    commits and issues with markdown text using the hash '\#'.

    -   Check "Allow edits from maintainers" to allow other collaborators to make 
    changes to your pull request and in the right hand side, assign a reviewer 
    (in this case, your internal creative partner or whomever you'd like to 
    engage in this pull request).
...
outlined in GitHub's How to Review Pull Requests. If no changes are required,
the reviewer will merge the branch.
