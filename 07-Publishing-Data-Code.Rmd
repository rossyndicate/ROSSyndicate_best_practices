...
The decision of which data repository to use is less straight forward. There are
benefits and tradeoffs to each. The [Environmental Data Initiative
...
thorough metadata regardless of the metadata requirements of the data repository
chosen. For one-time uploads to EDI, you can use the
...
is useful. If creating metadata for other data repositories, review the [content
of the
...
metadata to Zenodo. While there are no requirements for publishing to Zenodo, it
is a best practice to include robust metadata alongside your code release. A
...
except you! Someone reading the Release Title should be able to discern what the
repository does. You can explain the 'how' in the description.
...
repository best practices include using an MIT use license, the LICENSE.txt file
...
top of the primary README file within the GitHub user interface on the web. This
addition to the repository does not need to be reviewed.
...
geographical (data are only from one location), contextual (data are only from a
single source), or implicit (data have been altered for a specific purpose). As
you prepare your data and metadata, it is important to think about the biases we
contribute to a given dataset.

To familarize yourself with bias that we introduce into our data and analyses,
